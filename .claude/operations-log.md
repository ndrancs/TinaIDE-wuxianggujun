...
## Stage 3 - Tree-sitter Completion 注入 (2025-12-05 15:30 - 16:00)

### 背景
- Native Completion 结果已经可以通过 `NativeLspRequestBridge` 拿到，但 EditorAutoCompletion 仍然依赖 Tree-sitter 默认实现（即空结果），C/C++ 面板不会弹出任何候选项。
- 需要将 Native completion 注入 Sora 自动补全面板，同时让 Language 层自带上下文（file/project path），避免 UI 手动拉取。

### 实施
1. **EditorDocumentExtras**
   - 新增 `EditorDocumentExtras.create()`，统一封装 `KEY_FILE_PATH/KEY_PROJECT_PATH`，通过 `CodeEditor.setText(text, extras)` 注入在 CompletionThread 可见的 extras。
   - `EditorFragment.loadFileContent()` 改为在加载文本时写入 extras，避免后续语言层反查文件路径。
2. **NativeAware TsLanguage**
   - `CppTreeSitterLanguageProvider` 现在返回 `NativeAwareCppLanguage`，覆写 `requireAutoComplete`。
   - `CppNativeCompletionDispatcher` 将 `ContentReference` + `CharPosition` 转换为 prefixLength，调用 `NativeLspService.requestCompletionAsync()`，并将结果映射为 `SimpleCompletionItem`。
   - 引入 `CompletionItemKind` 映射，自动为不同 kind 设置图标/颜色；deprecated 项在描述后追加 “(deprecated)”。
3. **Native Client guard**
   - requireAutoComplete 内部在发送请求前调用 `NativeLspService.nativeIsInitialized()`，必要时按 `projectPath` 初始化，保证补全与文档桥使用同一上下文。

### 效果
- Tree-sitter 语言现在直接驱动 Sora 的自动补全面板，Mock/Real 模式都会展示 Native 返回的候选。
- CompletionThread 依旧运行在后台，`publisher.updateList(true)` 确保面板及时刷新，支持 filter/sort。
- Stage 3 “Native Completion/Definition UI 接管” 中的补全面板部分完成，后续仅需将 Definition/References 注入 UI 即可。
