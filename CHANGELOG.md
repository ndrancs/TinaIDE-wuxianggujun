...
- 修复 0.18.5 在启动阶段可能误触发 Linux/PRoot 资源准备，导致多次闪退、卡住后才能进入并导出日志的问题；默认编译链路保持 native tina-toolchain，PRoot/Linux distro 继续作为可选环境。
- 修复 NDK API 35 shared library 模板运行时链接失败的问题，CMake 链接策略改为按目标解析依赖，避免新增库时继续硬编码系统库。
- 修复构建日志面板长按时系统菜单抢占自定义菜单的问题，并将复制、清空、全选等操作收敛到日志内容区长按菜单。
- 修复大纲面板被输入法整体顶起后底部出现空白、空大纲仍能点击空白 item，以及 item 间距过大的问题。
- 修复终端运行结束后已有输入会导致 Enter 只换行、不能关闭当前运行界面的问题。
- 修复未保存退出弹窗操作层级突兀的问题，移除多余取消按钮，仅保留不保存退出与全部保存并退出。
- 增加 release 混淆资源安全规则，保留 R 内部类与字段名，降低动态资源访问在 R8 混淆后崩溃的风险。

### Changed

- 构建日志底栏 Tab 支持角标显示日志条数，减少独立顶部工具栏占用空间。
- 默认开启 MT 数据文件提供能力，降低日志无法上传且用户手动打不开目录的概率。
- 更新 Linux 存储、MT 数据文件 Provider 与当前文档状态说明。

### Verification

- 已新增 NDK shared library 模板与 CMake 链接策略回归测试覆盖。
- 已做静态校验与 i18n 资源检查；本次按要求不执行 Gradle 编译。
