...
function _instance:tool(toolkind)
    ...
    if os.getenv("TINA_BRIDGE_ONLY") == "y" then
        local program = self:_android_bridge_virtual(toolkind)
        if program then
            return program, path.filename(program)
        end
        utils.raise("TinaIDE: toolkind(%s) not supported in bridge mode", toolkind)
        return
    end
...
function _instance:_android_bridge_virtual(toolkind)
    local defaults =
    {
        cc   = "clang",
        cxx  = "clang++",
