...
- ✅ `NativeLoader.kt` - 负责库加载（c++_shared → LLVM → clang-cpp → native_compiler）
- ✅ `SysrootInstaller.kt` - 首次运行解压 sysroot
- ✅ `NativeCompiler.kt` - JNI 编译接口封装
- ✅ `native_compiler.cpp` - C++ 前端实现（emitObj、syntaxCheck）

#### 3. 构建工具链

- ✅ Docker 构建环境（`docker/embedded-ndk/`）
- ✅ 统一同步脚本（`tools/sync-embedded-ndk.ps1`）
- ✅ 清理脚本（`docker/embedded-ndk/clean-local.ps1`）

### 🚧 进行中

- [ ] 恢复 `syntaxCheck()` - 需要放置 Clang C++ 头文件到 `external/embedded-ndk-libs/common-headers/clang`
...
- [ ] **Step 1**: 放置 Clang 头文件
  - 执行：`tools/sync-embedded-ndk.ps1 -Abi arm64-v8a -ApiLevel 24`
  - 产物：`external/embedded-ndk-libs/common-headers/clang/`
...
```powershell
# 构建 arm64-v8a 版本（真机）
./docker/embedded-ndk/build-local.ps1 -Mode libs -Abi arm64-v8a -ApiLevel 24

# 构建 x86_64 版本（模拟器）
./docker/embedded-ndk/build-local.ps1 -Mode libs -Abi x86_64 -ApiLevel 24
```

产物位置：`external/embedded-ndk-libs/<abi>/`
...
- ✅ 统一使用 `sync-embedded-ndk.ps1` 同步产物
- ❌ 避免手工复制文件
...
│       └── clean-local.ps1
├── tools/
│   ├── sync-embedded-ndk.ps1              # 统一同步脚本
│   └── sync-llvm-headers.ps1              # 同步 LLVM 头文件
└── docs/
    ├── CLANG_INTEGRATION_ROADMAP.md       # 本文档
    ├── EMBEDDED_CLANG_STATUS.md           # 状态报告
    ├── EMBEDDED_NDK_SHARED_LIBS.md        # 共享库方案
    ├── EMBEDDED_NDK_TOOLS.md              # 工具链方案
    └── EMBEDDED_NDK_TOOLS_DOCKER.md       # Docker 构建指南
```
...
# 同步到项目
pwsh tools/sync-embedded-ndk.ps1 -Abi x86_64 -ApiLevel 24
```
...
- [EMBEDDED_NDK_SHARED_LIBS.md](./EMBEDDED_NDK_SHARED_LIBS.md) - 共享库方案详解
- [EMBEDDED_NDK_TOOLS.md](./EMBEDDED_NDK_TOOLS.md) - 工具链方案说明
- [EMBEDDED_NDK_TOOLS_DOCKER.md](./EMBEDDED_NDK_TOOLS_DOCKER.md) - Docker 构建指南
