...
- 开源版已经移除账号登录、第三方登录、激活码/许可证、会员和官方 AI 额度入口；私有后端与管理端不再随公开仓库分发。
- 近期最值得优先补齐：分屏编辑、用户自定义项目模板、扩充内置 C/C++ 片段库、插件系统的快捷键/工具栏/设置页（书签基础能力已实现，后续以体验优化为主）。
...
| 服务端能力 | 反馈、插件、下载和包管理等客户端能力保留；私有后端源码不公开 | `core/network`, `feature/help`, `core/plugin`, `core/packages` |
