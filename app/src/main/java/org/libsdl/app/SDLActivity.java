...
*/
    ...
    /**
     * Returns the expected native SDL version for Java/C compatibility checks.
     * Derived classes may override this to relax/adjust compatibility policy.
     */
    protected String getExpectedSdlVersion() {
        return String.valueOf(SDL_MAJOR_VERSION) + "." +
               String.valueOf(SDL_MINOR_VERSION) + "." +
               String.valueOf(SDL_MICRO_VERSION);
    }

    /**
     * Returns whether native SDL version is compatible with this Java layer.
     * The default implementation requires exact version match.
     */
    protected boolean isNativeVersionCompatible(String version) {
        return getExpectedSdlVersion().equals(version);
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!mBrokenLibraries) {
            String expectedVersion = getExpectedSdlVersion();
            String version = nativeGetVersion();
            if (!isNativeVersionCompatible(version)) {
                mBrokenLibraries = true;
    ...
    private void unregisterBackInvokedCallback() {
        ...
        getOnBackInvokedDispatcher().unregisterOnBackInvokedCallback(
                (OnBackInvokedCallback) mBackInvokedCallback);
    ...
    protected void performSystemBack() {
        if (!isFinishing()) {
            super.onBackPressed();
        }
    }
